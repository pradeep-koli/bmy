...
trait TimeTrait
{
     ...
     * @throws Exception
     */
    public static function createFromDate(?int $year = null, ?int $month = null, ?int $record = null, $timezone = null, ?string $locale = null)
    {
        return static::create($year, $month, $record, null, null, null, $timezone, $locale);
    }
    ...
    public static function create(
        ...
        return new self(date('Y-m-d H:i:s', strtotime("{$year}-{$month}-{$record} {$hour}:{$minutes}:{$seconds}")), $timezone, $locale);
    }
    ...
    {
        [$year, $month, $record, $hour, $minute, $second] = explode('-', $this->format('Y-n-j-G-i-s'));
    ...
    /**
     * Returns a new Time instance with $records added to the time.
     *
     * @return static
     */
    public function addDays(int $records)
    {
        $time = clone $this;

        return $time->add(DateInterval::createFromDateString("{$records} days"));
    }
    ...
    /**
     * Returns a new Time instance with $records subtracted from the time.
     *
     * @return static
     */
    public function subDays(int $records)
    {
        $time = clone $this;

        return $time->sub(DateInterval::createFromDateString("{$records} days"));
    }
    ...
    public function humanize()
    {
        ...
        $records    = $now->fieldDifference($time, IntlCalendar::FIELD_DAY_OF_YEAR);
        ...
        if ($years !== 0) {
            ...
            $before = $months < 0;
        } elseif ($records !== 0 && (abs($records) >= 7)) {
            $weeks  = ceil($records / 7);
            ...
            $phrase = lang('Time.days', [abs($records)]);
        } elseif ($hours !== 0) {
