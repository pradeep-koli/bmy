...
<body style="box-shadow: 20px 20px 50px #edeef1;">
  ...
  <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script> -->
                    ...
                    $('#calendar').fullCalendar({
                        ...
                        events: [
                            <?php foreach ($events as $event): ?>
                                <?php
                                // Apply role-based event filtering
                                if ((session('role') == "manager" && $event['token'] == session('name')) ||
                                    session('role') == "admin" ||
                                    (session('role') == "user" && $event['username'] == session('name'))
                                ) : ?> {
                            ...
                            <?php foreach ($yachtNA as $holiday): ?>
                                <?php
                                // Apply role-based filtering for holiday events
                    ...
                    <div class="card-body" style="border: 2px solid #e9e9ec;">
                        ...
                        <form method="post" id="myForm" onsubmit="return validateForm()" class="mb-3"
                            ...
                            <div class="row mb-3">
                                ...
                                <select class="form-select" name="hname" id="yachtSelect" required onchange="updateYachtInfo()">
                                    <option value="">Select Yacht</option>
                                    <?php foreach ($options as $option): ?>
                                        <?php if ($option['username'] == session('name') || 'admin' == session('role')): ?>
                                            <option value="<?php echo $option['hname']; ?>" data-id="<?php echo $option['id']; ?>" data-type="<?php echo $option['yacht_type']; ?>" data-price="<?php echo $option['price']; ?>">
                                                <?php echo $option['hname']; ?>
                                            </option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <input type="hidden" name="yacht_id" id="yachtId">
                            <input type="hidden" name="yacht_type" id="yachtType">
                            <input type="hidden" name="price" id="yachtPrice">
            ...
            <!-- Function to update the hidden input with the yacht ID and yacht type based on the selected yacht name -->
            <script>
                function updateYachtInfo() {
                    var selectedOption = document.getElementById('yachtSelect').selectedOptions[0];
                    var yachtId = selectedOption.getAttribute('data-id');
                    var yachtType = selectedOption.getAttribute('data-type');
                    var yachtPrice = selectedOption.getAttribute('data-price');
                    document.getElementById('yachtId').value = yachtId;
                    document.getElementById('yachtType').value = yachtType;
                    document.getElementById('yachtPrice').value = yachtPrice;
                }
            </script>
